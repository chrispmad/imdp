clean_excel_datetime_cols = function(dat, buffer_minutes = 30) {
  ...
  adjust_times <- function(df, time_col = "TimeOfInspection",
    ...
    df <- df |>
      dplyr::mutate(
        # Ensure datetimes
        !!rlang::sym(time_col) := lubridate::as_datetime(!!rlang::sym(time_col)),
        !!rlang::sym(start_col) := lubridate::as_datetime(!!rlang::sym(start_col)),
        !!rlang::sym(end_col)   := lubridate::as_datetime(!!rlang::sym(end_col)),

        # Convert to Vancouver then UTC
        Inspection.Datetime_VAN = lubridate::with_tz(!!rlang::sym(time_col), tzone = "America/Vancouver"),
        Inspection.Datetime_UTC = lubridate::with_tz(Inspection.Datetime_VAN, tzone = "UTC"),

        inspection_ok = Inspection.Datetime_UTC >= ( !!rlang::sym(start_col) - lubridate::minutes(buffer_minutes) ) &
          Inspection.Datetime_UTC <= ( !!rlang::sym(end_col) + lubridate::minutes(buffer_minutes) ),

        Inspection.Datetime_VAN = dplyr::if_else(!inspection_ok,
  ...
  dat[[3]] <- dat[[3]] |>
    ...
    dplyr::mutate(dplyr::across(c(`Date + Time of Inspection`, shift_start, `Shift End Time`),
                                ~ openxlsx::convertToDateTime(as.numeric(.)))) |>
    dplyr::mutate(`Shift Date` = as.Date(as.numeric(`Shift Date`), origin = "1899-12-30")) |>
    dplyr::mutate(Hour_Min_of_Insp = stringr::str_extract(TimeOfInspection, '(?<= )[0-9]{2}:[0-9]{2}(?=:)'))
